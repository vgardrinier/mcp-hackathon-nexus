...
The MCP server now reads MCP server definitions from YAML under `apps/mcp/servers/**/config.yml`. You no longer need Supabase or any auth/dashboard setup to boot the MCP service.
...
Use the HTTP endpoint `http://localhost:3001/mcp`. For Cursor, update your MCP settings JSON accordingly (no Authorization header needed).
